...
{
    if (key == GLFW_KEY_RIGHT && action == GLFW_PRESS) { GLFEM_ACTION = 1; }
    if (key == GLFW_KEY_LEFT && action == GLFW_PRESS)  { GLFEM_ACTION = -1; }
...
void glfemDrawElement(float *x, float *y, int n)
{
    ...
    for (j = 0; j < n; j++) { glVertex2f(x[j], y[j]); }
    glVertex2f(x[0], y[0]);
    glEnd();
}
