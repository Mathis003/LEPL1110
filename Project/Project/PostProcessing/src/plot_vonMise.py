...
if __name__ == "__main__":
    ...
    nameMesh = "../../Processing/data/mesh.txt"
    mesh = Mesh(nameMesh)
    nameVonMise = "../../Processing/data/vonMises.txt"
    vonMise = np.loadtxt(nameVonMise)

    nameSolution = "../../Processing/data/UV.txt"
    uv = np.loadtxt(nameSolution, skiprows=1, delimiter=",")
    uv_norm = np.linalg.norm(uv, axis=1)

    factor = 2e4

    cb = mesh.plotfield(vonMise, uv*factor, cmap="cool")
    plt.colorbar(cb)
    ...
    # plt.show()
    plt.savefig("../../Processing/data/vonMise.pdf", bbox_inches='tight')
