...
{  
    ...
    {
        double theSoluceX = theSoluce[2 * i + 0];
        double theSoluceY = theSoluce[2 * i + 1];
        theNodes->X[i] += theSoluceX * deformationFactor;
        theNodes->Y[i] += theSoluceY * deformationFactor;
        normDisplacement[i] = sqrt(theSoluceX * theSoluceX + theSoluceY * theSoluceY);
    }
